...
1. TanStack Query narrow adoption (`docs/floci-ui-inspiration.md` Target C): **spike on branch `feat/tanstack-query-spike`** — adopted for `runtime.get` poll + `deployments.list` only; workspace snapshots stay on existing hooks
