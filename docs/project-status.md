...
**Post-v0.9.16 on `dev` (not yet released):** Azure Storage Queue send (`azure.queues.sendMessage`). App Service log workspace dropdown loads with web apps inventory. SQS send keeps the draft when the request fails.
