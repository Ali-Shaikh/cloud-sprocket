...
import {
  ...
  kqlQueryHasNextPage,
  stripAggregateIncompatibleClauses,
...
export { stripAggregateIncompatibleClauses };
...
export function wafQueryHasNextPage(rowCount: number, pageSize: number): boolean {
  return kqlQueryHasNextPage(rowCount, pageSize);
}

/** Trim the extra probe row used to detect further pages. */
export function trimWafQueryPageRows<T>(rows: T[], pageSize: number): T[] {
  return trimKqlQueryPageRows(rows, pageSize);
}

export const WAF_PAGE_SIZE_OPTIONS = KQL_PAGE_SIZE_OPTIONS;