...
variable "backend_source_dir" {
  type        = string
  description = "Directory containing your Node processor with a handler.handler export. Leave as the default to deploy the sample processor."
  default     = "./sample-processor"
}

variable "lambda_memory_mb" {
  type        = number
  description = "Memory for the processor Lambda, in megabytes."
  default     = 128
}
