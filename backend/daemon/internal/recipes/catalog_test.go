...
func TestBundledCatalogV07(t *testing.T) {
	...
	if len(manifests) < 22 {
		t.Fatalf("expected at least 22 bundled recipes, got %d", len(manifests))
	}
	...
	if counts[KindAppDeploy] < 16 {
		t.Fatalf("expected at least 16 app-deploy recipes, got %d", counts[KindAppDeploy])
	}
